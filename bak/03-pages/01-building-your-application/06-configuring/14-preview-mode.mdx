...
> **注意事项**: during rendering `next/router` exposes an `isPreview` flag, see the [router object docs](/docs/pages/api-reference/functions/use-router#router-object) for more info.
...
```js title="pages/api/clear-preview-mode-cookies.js"
export default function handler(req, res) {
...
> **注意事项**: To test Preview Mode locally over HTTP your browser will need to allow third-party cookies and local storage access.
