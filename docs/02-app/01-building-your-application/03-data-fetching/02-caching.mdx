...
title: 缓存数据
nav_title: 缓存
description: 在Next.js中学习缓存路由。
---

import Tabs from '@theme/Tabs'
import TabItem from '@theme/TabItem'

Next.js内置了对缓存数据的支持，既可以基于每个请求(推荐)，也可以针对整个路由段。
...
```ts title="utils/getUser.ts" switcher
...
```js title="utils/getUser.js" switcher
import { cache } from 'react'
...
```tsx title="app/user/[id]/layout.tsx" switcher
...
```jsx title="app/user/[id]/layout.js" switcher
import { getUser } from '@utils/getUser'
...
```tsx title="app/user/[id]/page.tsx" switcher
import { getUser } from '@utils/getUser'
...
```jsx title="app/user/[id]/page.js" switcher
import { getUser } from '@utils/getUser'
...
```tsx title="components/User.tsx" switcher
import { getUser } from '@utils/getUser'
...
```jsx title="components/User.js" switcher
import { getUser } from '@utils/getUser'
...
> **注意事项**: We recommend using per-request caching for improved granularity and control over caching.
