...
Next.js内置了对CSS模块的支持，扩展名为`.module.css`。

CSS模块通过自动创建唯一的类名来局部作用域CSS。
这允许您在不同的文件中使用相同的类名，而不用担心冲突。
这种行为使得CSS模块成为包含组件级CSS的理想方式。
...
```csstitle="app/dashboard/styles.module.css"
.dashboard {
  padding: 24px;
}
```
...
> **注意事项**: This is different from the `pages` directory, where you can only import global styles inside the `_app.js` file.
...
> **注意事项**: External stylesheets must be directly imported from an npm package or downloaded and colocated with your codebase. You cannot use `<link rel="stylesheet" />`.
