...
Next.js 内置了对环境变量的支持，它允许你做以下事情:

- [使用`.env.local`来加载环境变量](#loading-environment-variables)
- [使用`NEXT_PUBLIC_`前缀捆绑浏览器的环境变量](#bundling-environment-variables-for-the-browser)

## 加载环境变量

Next.js内置了将环境变量从`.env.local`加载到`.process.env`的支持。
...
> **注意事项**: If you need to use variable with a `$` in the actual value, it needs to be escaped e.g. `\$`.
...
> **注意事项**: `.env`, `.env.development`, and `.env.production` files should be included in your repository as they define defaults. **`.env*.local` should be added to `.gitignore`**, as those files are intended to be ignored. `.env.local` is where secrets can be stored.
...
> **注意事项**: similar to Default Environment Variables, `.env.test` file should be included in your repository, but `.env.test.local` shouldn't, as `.env*.local` are intended to be ignored through `.gitignore`.
...
> **注意事项**: The allowed values for `NODE_ENV` are `production`, `development` and `test`.

## 很高兴知道

- 如果您正在使用 [`/src` 目录](/docs/app/building-your-application/configuring/src-directory), `.env.*` 文件应保留在项目的根目录中。
- 如果环境变量`NODE_ENV`未分配, 当运行`next dev`命令时，Next.js会自动分配`development`，或者为所有其他命令分配`production`。
