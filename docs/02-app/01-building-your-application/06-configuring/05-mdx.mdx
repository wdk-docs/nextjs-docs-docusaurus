...
有两个流行的社区包用于获取MDX内容: [`next-mdx-remote`](https://github.com/hashicorp/next-mdx-remote#react-server-components-rsc--nextjs-app-directory-support) and [`contentlayer`](https://www.contentlayer.dev/).
...
`@next/mdx`默认情况下**不**支持frontmatter，尽管有许多解决方案可以将frontmatter添加到mdx内容中，例如[gray-matter](https://github.com/jonschlinkert/gray-matter)。

要使用`@next/mdx`访问页面元数据，你可以从`.mdx`文件中导出一个元对象:
