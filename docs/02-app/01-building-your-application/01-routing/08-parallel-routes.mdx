---
title: 并行路由
description: 在可独立导航的同一视图中同时呈现一个或多个页面。用于高度动态应用程序的模式。
---

import Tabs from '@theme/Tabs'
import TabItem from '@theme/TabItem'

并行路由允许您同时或有条件地呈现相同布局中的一个或多个页面。
对于应用程序中高度动态的部分，比如社交网站上的仪表板和提要，并行路由可以用来实现复杂的路由模式。
...
并行路由允许您为每个路由定义独立的错误和加载状态，因为它们是独立地流进来的。
...
并行路由还允许您根据某些条件(如身份验证状态)有条件地呈现插槽。这使得同一URL上的代码完全分离。
...
> **注意事项**: The `children` prop is an implicit slot that does not need to be mapped to a folder. This means `app/page.js` is equivalent to `app/@children/page.js`.
...
```tsx title="app/@authModal/login/page.tsx" switcher
...
```jsx title="app/@authModal/login/page.js" switcher
import { Modal } from 'components/modal'
...
```tsx title="app/@authModal/default.tsx" switcher
export default function Default() {
  return null
}
```

```jsx title="app/@authModal/default.js" switcher
export default function Default() {
  return null
}
```
...
```tsx title="app/@authModal/login/page.tsx" highlight="5" switcher
'use client'
...
```jsx title="app/@authModal/login/page.js" highlight="5" switcher
'use client'
import { useRouter } from 'next/navigation'
import { Modal } from 'components/modal'
...
```tsx title="app/@authModal/[...catchAll]/page.tsx" switcher
export default function CatchAll() {
  return null
}
```

```jsx title="app/@authModal/[...catchAll]/page.js" switcher
export default function CatchAll() {
  return null
}
```
