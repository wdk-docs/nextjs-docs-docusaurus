...
`opengraph-image`和`Twitter -image`文件约定允许你为一个路由段设置Open Graph和Twitter图像。

当用户分享到你的网站的链接时，它们对于设置出现在社交网络和消息应用程序上的图像很有用。

有两种方法可以设置Open Graph和Twitter图片:

- [使用映像文件 (.jpg, .png, .gif)](#image-files-jpg-png-gif)
- [使用代码生成图像 (.js, .ts, .tsx)](#generate-images-using-code-js-ts-tsx)

## 图像文件 (.jpg, .png, .gif)

通过在段中放置一个`opengraph-image`或`twitter-image`图像文件，使用图像文件来设置路由段的共享图像。

Next.js将评估该文件，并自动为应用程序的`<head>`元素添加适当的标签。
...
添加一个`opengraph-image.(jpg|jpeg|png|gif)`的图像文件到任何路由段。
...
添加一个`twitter-image.(jpg|jpeg|png|gif)`图像文件到任何路由段。
...
```txttitle="opengraph-image.alt.txt"
About Acme
```
...
```txttitle="twitter-image.alt.txt"
About Acme
```
