...
> **很高兴知道**: 你可以在你的新项目中使用[Pages Router](/docs/pages)。我们建议使用App Router启动新应用，以利用React的最新特性。
...
> **很高兴知道**: 如果你忘记创建`layout.tsx`， Next.js会在运行带有`next dev`的开发服务器时自动为你创建这个文件。
...
> **很高兴知道**: 虽然你可以在同一个项目中同时使用这两个路由器，但`app`中的路由会优先于`pages`中的路由。我们建议在你的新项目中只使用一个路由器，以避免混淆。
