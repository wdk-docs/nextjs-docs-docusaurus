...
**Note**: This is an old blog post. [ZEIT is now
Vercel](https://rauchg.com/2020/vercel). An “`@mdx` pragma” is no longer
needed. The below is kept as is for historical purposes.

:::note

It’s been a year and a half since the first MDX commit and a year since MDX was first
announced at ZEIT Day. MDX is a radical paradigm shift in how to write immersive
content using components. It’s an open, [authorable format](https://johno.com/authorable-format)
that makes it _fun_ to write again.
